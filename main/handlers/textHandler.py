...
def text_processing(bot, message, my_keyBoard, array):
    ...
    if len(array)!=0:
        ...
        while y<len(array):
            ...
            #photo next step 
            elif array[y].id_chat==message.chat.id and array[y].blockName=="🔴 інформація про правопорушення" and array[y].textDrugs!=0 and array[y].photoDrugs==0 and array[y].gpsAboutDrugs==0:
        ...
        bot.send_message(message.chat.id, ' Я не знаю що відповісти😢 2\n Виберіть та натисніть\n\n👉 📖 користувачам боту\n\n👉 🔴 повідомити про правопорушення \n\n👉🔺  /start')
